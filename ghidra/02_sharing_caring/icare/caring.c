...
void CARINGUpdate(CARING_CTX *context, unsigned char *input, unsigned int inputlen)
{
	...
	if(inputlen >= partlen)
	{
		...
		CARINGTransform(context->state, context->buffer);

		for(i = partlen; i+64 <= inputlen; i+=64)
			CARINGTransform(context->state, &input[i]);
...
void CARINGFinal(CARING_CTX *context, unsigned char digest[16])
{
	...
	CARINGEncode(bits, context->count, 8);
	CARINGUpdate(context, PADDING, padlen);
	CARINGUpdate(context, bits, 8);
	CARINGEncode(digest, context->state, 16);
}
